...
<script>
    export default {
        ...
        wrappers.forEach(wrapper=>{
            ...
            const dateformated = moment(wrapper.shipmentdetails.pickupdate).format(format3);
            wrapper.pickupdate = dateformated;

        })
          const formattedWrappers = wrappers.map(function(wrapper) {
        return {
            width:  wrapper.width.replace(/,/g, '.'),
            height: wrapper.height.replace(/,/g, '.'),
            weight: wrapper.weight.replace(/,/g, '.'),
            length: wrapper.length.replace(/,/g, '.'),
            ...
            pickupdate: wrapper.pickupdate
        }
      });
      console.log("formated wrappers for quote ",formattedWrappers);
      ...
      .then(data => {
          ...
          console.log("table contenant les prix",table_for_price);
          ...
          /*passe the total price to shipment*/
          wrappers.forEach((wrapper)=>{
            wrapper.shipmentdetails.tatalprice=this.sum_for_total_price_all_taxe;
            wrapper.packageprice=this.sum_for_total_price_all_taxe;
          });
          //end passing price to shipment
      })
      .catch(error=>{
